...
export default function OrdersForm() {
  ...
  const onOptionSelected = useCallback((option: { id: string | number, label: string, value: IAlumnRecord } | null) => {
  ...
  const onProductSelected = useCallback((option: { id: string | number, label: string, value: IProductRecord } | null) => {
    if (option) {
      setProducts(currentProducts => {
        if (currentProducts.some(product => product.id === option.value.id)) return currentProducts;

        setErrors([])
        return [...currentProducts, { id: option.value.id, name: option.value.name, quantity: "1", price: option.value.price }];
      });
    }
  }, [])
  ...
  const fetchAlumns = useCallback(async (query: string) => {
  ...
  const orderTotal = useMemo(() => calculateOrderTotal(products), [products]);
      ...
      <main className="max-w-4xl space-y-stack-md">
        <section className="bg-surface-lowest rounded-xl shadow-soft border border-surface-container p-6">
          ...
          <div className="relative [&_input]:pl-12 [&_input]:pr-4 [&_input]:py-3 [&_input]:rounded-lg [&_input]:border-outline-variant [&_input]:bg-surface-lowest [&_input]:text-on-surface [&_input]:focus:ring-1 [&_input]:focus:ring-primary [&_input]:focus:border-primary [&_input]:text-body-md [&_input]:font-body-md [&_ul]:rounded-lg [&_ul]:border-outline-variant [&_ul]:bg-surface-lowest [&_ul]:text-on-surface [&_ul]:shadow-soft">
            ...
            <AutocompleteCombobox fetchOptions={fetchAlumns} onSelect={onOptionSelected} onInputChange={() => setAlumnId(0)} placeholder="Search alumns by name or ID..." />
          </div>
        </section>
        ...
        <section className="bg-surface-lowest rounded-xl shadow-soft border border-surface-container p-6">
          ...
          <div className="space-y-4 mb-8">
            <div className="relative [&_input]:pl-12 [&_input]:pr-4 [&_input]:py-3 [&_input]:rounded-lg [&_input]:border-outline-variant [&_input]:bg-surface-lowest [&_input]:text-on-surface [&_input]:focus:ring-1 [&_input]:focus:ring-primary [&_input]:focus:border-primary [&_input]:text-body-md [&_input]:font-body-md [&_ul]:rounded-lg [&_ul]:border-outline-variant [&_ul]:bg-surface-lowest [&_ul]:text-on-surface [&_ul]:shadow-soft">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-outline pointer-events-none z-10">&#128269;</span>
              <SearchCombobox fetchOptions={fetchProducts} onSelect={onProductSelected} placeholder="Search products..." />
            </div>
