...
const categories = ["rent", "utilities", "payroll", "maintenance", "commission", "other"];
const paymentMethods = ["cash", "transfer", "card"];
const emptyForm = { amount: "", date: "", description: "", category: "rent", payment_method: "cash" };
...
function getRequestErrorMessage(error: unknown) {
  ...
  if (typeof data !== "object" || data === null) return "Unable to create expense.";

  if ("errors" in data && typeof data.errors === "object" && data.errors !== null) {
    return Object.values(data.errors).flat().join(" ");
  }

  return "error" in data && typeof data.error === "string" ? data.error : "Unable to create expense.";
}

export default function NewExpenseModal({ isOpen, onClose, onCreated, onError }: NewExpenseModalProps) {
  ...
  async function submitExpense(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true);
    try {
      await api.post("/expenses", { expense: { ...form, amount: Number(form.amount) } });
      setForm(emptyForm);
      onCreated();
      onClose();
    } catch (error: unknown) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => !isSaving && onClose()}>
          <div className="relative z-20 w-[92%] max-w-md bg-surface rounded-xl shadow-2xl flex flex-col overflow-hidden animate-[fadeIn_0.2s_ease-out]" onClick={event => event.stopPropagation()}>
            <div className="flex items-center justify-between px-gutter py-4 border-b border-outline-variant">
              <h2 className="font-headline-sm text-headline-sm text-on-surface">New Expense</h2>
              <button
                type="button"
                onClick={onClose}
                disabled={isSaving}
            ...
            <form id="new-expense-form" onSubmit={submitExpense} className={`p-gutter flex flex-col gap-stack-md overflow-y-auto max-h-[70vh] ${isSaving ? "opacity-50 pointer-events-none" : ""}`}>
              <div className="flex flex-col gap-base">
                <label htmlFor="expense_amount" className="font-label-md text-label-md text-on-surface-variant">Amount</label>
                <input id="expense_amount" required min="0.01" step="0.01" type="number" name="amount" value={form.amount} onChange={updateField} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-body-md text-body-md text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-shadow placeholder-outline" />
              </div>
              <div className="flex flex-col gap-base">
                <label htmlFor="expense_date" className="font-label-md text-label-md text-on-surface-variant">Date</label>
                <input id="expense_date" required type="date" name="date" value={form.date} onChange={updateField} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-body-md text-body-md text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-shadow" />
              </div>
              <div className="flex flex-col gap-base">
                <label htmlFor="expense_description" className="font-label-md text-label-md text-on-surface-variant">Description</label>
                <input id="expense_description" required type="text" name="description" value={form.description} onChange={updateField} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-body-md text-body-md text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-shadow" />
              </div>
              <div className="flex flex-col gap-base">
                <label htmlFor="expense_category" className="font-label-md text-label-md text-on-surface-variant">Category</label>
              ...
              <div className="flex flex-col gap-base">
                <label htmlFor="expense_payment_method" className="font-label-md text-label-md text-on-surface-variant">Payment Method</label>
                <select id="expense_payment_method" name="payment_method" value={form.payment_method} onChange={updateField} className="w-full bg-surface-container-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-body-md text-body-md text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-shadow">
